...
analyze_correlation = function(name, alpha, fractions) {
    ...
    for (frac in fractions) {
        ...
        print(sprintf("matrix %s, p-value %.2f, cutting at %.2f, %d elements (%.1f %% ),", name, alpha, frac,
                      sum( ! is.na(corrs_m2)), 
                      sum( ! is.na(corrs_m)) / (dim(corrs_m2)[[1]] * dim(corrs_m)[[2]])*100))
...
analyze_correlation("enriched", 0.05, c(0.2, 0.15))
