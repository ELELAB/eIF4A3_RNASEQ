...
corrs = read.table("correlation_enriched.tsv", stringsAsFactors=FALSE, sep='\t', header=TRUE)
pvals = read.table("p-values_enriched.tsv", stringsAsFactors=FALSE, sep='\t', header=TRUE)
...
png("heatmap_correlations_full_TPonly_enriched.png", width=1000, height=800)
Heatmap(t(corrs_m), 
        col=colf,
        na_col="grey", 
        cluster_columns=F,
...
png("distribution_correlations_full_TPonly_enriched.png")
...
ha = columnAnnotation(samples = anno_text(corrs$nsamples))
png("heatmap_correlations_0.05_TPonly_enriched.png", width=1000, height=800)
Heatmap(t(corrs_m), 
        col=colf,
        na_col="grey", 
        cluster_columns=F,
...
png("distribution_correlations_0.05_TPonly_enriched.png")
ggplot(values, aes(v1)) + 
    geom_histogram(aes(y=..density..),na.rm=TRUE) +
    geom_density(alpha=.2, fill="#FF6666")  # Overlay with transparent density plot
dev.off()
...
ha = columnAnnotation(samples = anno_text(corrs$nsamples))
png("heatmap_correlations_full_TPonly_not_filtered_enriched.png", width=1000, height=800)
Heatmap(t(corrs_m), 
        col=colf,
        na_col="grey", 
        cluster_columns=F,
...
png("distribution_correlations_full_TPonly_not_filtered_enriched.png")
ggplot(values, aes(v1)) + 
    geom_histogram(aes(y=..density..), na.rm=TRUE) +
    geom_density(alpha=.2, fill="#FF6666")  # Overlay with transparent density plot
dev.off()
